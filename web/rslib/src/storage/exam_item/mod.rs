...
//! rather than through a schema upgrade: bumping the collection's schema
//! version would make the file unreadable to other Anki builds, and this record
//! is not part of the synced collection. It is emphatically *not* the revlog --
//! the revlog holds the scheduler's self-rated reviews, and an
//! objective-correctness record is a different kind of claim.
...
/// [`crate::readiness::evidence`] uses. A note is an exam item when one of
/// those tags is exactly [`EXAM_ITEM_TAG`]; its topics are the
/// `mcat::`-prefixed tags on the same note.
///
...
impl SqliteStorage {
    ...
    ///
    /// Idempotent, and cheap enough to call before every access, which keeps
    /// the table available on collections created before this feature
    /// existed without touching the schema version.
